...
title Ximi Unlock BL - Snapdragon 8 Gen 3 (SM8650)

echo %CYN%=================================================================%RST%
echo  Ximi Tool Lite - Unlock Bootloader Snapdragon 8 Gen 3 (SM8650)
...
echo %CYN%==========================================%RST%
echo  %YEL%(Neu dien thoai chay HyperOS High Version, dung Unlock_8G3_HighVersion.bat)%RST%

set /p choice=Nhap so tuong ung roi an Enter:
if "%choice%"=="1" ( set "factoryImages=Redmik70pro" & set "unlockGPT=Redmik70pro" )
if "%choice%"=="2" ( set "factoryImages=Redmik80"    & set "unlockGPT=Redmik80" )
if "%choice%"=="3" ( set "factoryImages=Xiaomi14"    & set "unlockGPT=Xiaomi14" )
if "%choice%"=="4" ( set "factoryImages=Xiaomi14pro" & set "unlockGPT=Xiaomi14pro" )
if "%choice%"=="5" ( set "factoryImages=Xiaomi14ultra" & set "unlockGPT=Xiaomi14ultra" )
...
if %errorlevel% neq 0 (
    echo %RED%[!] SELinux van Enforcing.%RST%
    echo %YEL%Neu may chay HyperOS High Version, chuyen sang Unlock_8G3_HighVersion.bat.%RST%
    pause
    exit /B 1
)
