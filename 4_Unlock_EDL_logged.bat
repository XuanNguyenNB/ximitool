@echo off
title Ximi Unlock EDL - Log Mode
cd /d "%~dp0"
if not exist "%~dp0reports\" mkdir "%~dp0reports\" >nul 2>nul

echo [LOG] Che do log da bat.
echo [LOG] Trace log se duoc luu tai: %~dp0reports\edl_trace_YYYYMMDD_HHMMSS.log
echo.
...
powershell -NoProfile -Command "Get-ChildItem '%~dp0reports\edl_trace_*.log' -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 3 | ForEach-Object { $_.FullName }"
echo =================================================================
echo.
